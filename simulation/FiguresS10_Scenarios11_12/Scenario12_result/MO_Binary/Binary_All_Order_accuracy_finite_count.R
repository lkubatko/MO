...
for (indexn in 1:100){
  ...
  all_pair_result[indexn,]=c(dim(all_match_all_branch_sub)[1],numSites,alpha,m10,missingindex=0,Genotype="Binary",lost_rate=0,lost_br=0,Method="MO")
  
}

all_pair_result_dat = rbind(all_pair_result_dat,all_pair_result)

all_pair_result_form_0_1_sub = sprintf('/fs/project/kubatko.2-temp/gao.957/workspace/cellcoal-master/cellcoal_50tips/MO_Binary/binary_all_order_matrix_01_0_1_out_finite_%s.csv',paraInd)
write.csv(all_pair_result,file=all_pair_result_form_0_1_sub)


}

all_pair_result_form_0_1 = sprintf('/fs/project/kubatko.2-temp/gao.957/workspace/cellcoal-master/cellcoal_50tips/MO_Binary/binary_all_order_matrix_01_0_1_out_finite.csv')
