...
for(parameter in 1:dim(parameter_setting)[1]){
  ...
  alpha =   parameter_setting[parameter,1]#fpr
  beta = parameter_setting[parameter,2]#fnr
  ...
  binary_folder_form = sprintf('/fs/project/kubatko.2-temp/gao.957/workspace/github_MO/Figures3_4_Scenarios1_2/Scenario1_result/MO_Binary/Binary_alpha0%s_beta0%s_result',alpha_str, beta_str)
  ...
  for (indexn in 1:1000){
    ...
    obs_form = sprintf('/fs/project/kubatko.2-temp/gao.957/workspace/github_MO/Figures3_4_Scenarios1_2/Scenario1/SimulateData_New/Binary_alpha0%s_beta0%s/binary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, alpha_str, beta_str,indexn)
    ...
    for (i in 1:dim(initial_obs_0_1_recode)[1]){
      ...
      #print(i)
      print(i)
      rgamma(10, shape = 2*10^(-0), scale = (10^(-0)/2))
      rd_unit_theta <- rbeta(10, (10^7)*unit_theta, (10^7)*(1-unit_theta))
      rd_unit_gamma <- rbeta(10, (10^9)*unit_gamma, (10^9)*(1-unit_gamma))
      ...
      for (j in 1:10){
        ...
        generate_prob_br_all_single <- MO_binary(alpha,beta,rd_unit_theta[j]+rd_unit_gamma[j],initial_obs_0_1_recode[i,],sampletr)
    ...
    binary_prob_matrix_all_0_1_out = sprintf("/fs/project/kubatko.2-temp/gao.957/workspace/github_MO/Figures3_4_Scenarios1_2/Scenario1_result/MO_Binary/Binary_alpha0%s_beta0%s_result/all_binary_prob_matrix_all_0_1_out_matrix%s.csv",alpha_str,alpha_str,indexn)
