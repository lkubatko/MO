...
for(paraInd in c(2,6,10)){
  ...
  location_acc = data.frame(matrix(ncol = 3, nrow = 99))
  start_time <- Sys.time()
  for (indexn in 1:2){
    print(indexn)
    ...
    binary_prob_matrix_all_0_1_out = sprintf("/fs/project/kubatko.2-temp/gao.957/workspace/cellcoal-master/cellcoal_50tips/MO_Binary/Binary_alpha0%s_beta0%s_%s_result/all_binary_prob_matrix_all_0_1_out_matrix%s.csv",alpha_str,alpha_str,numSites,indexn)
  ...
  end_time <- Sys.time()
  spend_time = difftime(time1 = end_time, time2 = start_time, units = "secs")
  print(c("MO Binary took",numSites,spend_time))
