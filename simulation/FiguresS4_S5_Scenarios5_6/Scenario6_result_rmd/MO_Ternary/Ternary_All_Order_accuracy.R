...
for(paraInd in 10:dim(parameter_setting)[1]){
...
for (indexn in 1:100){
  ...
  if (missingindex == 0){
    ...
    prob_form_0_1 = sprintf("./FiguresS4_S5_Scenarios5_6/Scenario6_result_rmd/MO_Ternary/Ternary_alpha0%s_beta0%s_%s_result/all_ternary_prob_matrix_all_0_1_2_out_matrix%s.csv",alpha_str,alpha_str,numSites,indexn)
    
  }else{
    
    prob_form_0_1 = sprintf("./FiguresS4_S5_Scenarios5_6/Scenario6_result_rmd/MO_Ternary/Ternary_alpha0%s_beta0%s_%s_result/all_ternary_prob_matrix_all_0_1_out_matrix%s_%s.csv",alpha_str,alpha_str,numSites,indexn,missingindex)
  ...
  all_true_all_branch = data.frame(matrix(, nrow=0, ncol=2))#all true possible pairs on true tree
  all_match_all_branch_sub = data.frame(matrix(, nrow=0, ncol=4))#inferred matched pair
  all_true_match_all_branch_sub = data.frame(matrix(, nrow=0, ncol=4))#true pairs
  ...
  colnames(all_true_all_branch)=c("first_br_indexn","second_br_indexn")
  all_true_all_branch_sub_all = na.omit(rbind(all_true_all_branch,
  ...
  colnames(all_true_all_branch_sub_all) = c("first_br_indexn","second_br_indexn")
    ...
    for (i in 1:(dim(mat_prob_form_0_1)[1]-1)){
      ...
      for (j in (i+1):dim(mat_prob_form_0_1)[1]){
        ...
        if(first_indexn == second_indexn & 
           mat_prob_form_0_1$selected_First_branch[i] == mat_prob_form_0_1$selected_First_branch[j] & 
           mat_prob_form_0_1$selected_First_branch[i] != 0 & mat_prob_form_0_1$selected_First_branch[j] != 0){
        ...
        }else if(first_indexn != second_indexn & 
                 mat_prob_form_0_1$selected_First_branch[i] != mat_prob_form_0_1$selected_First_branch[j]){
          ...
          matchresult_1 = match_df(all_true_all_branch_sub_all, br_indexn_1)#compare selected mutation br
          ...
          matchresult_2 = match_df(all_true_all_branch_sub_all, br_indexn_2)#compare selected mutation br
          ...
          if(dim(matchresult_1)[1]==0 & dim(matchresult_2)[1]==1 & dim(truematchresult_1)[1]==0 & dim(truematchresult_2)[1]==1){
          ...
          }else if(dim(matchresult_1)[1]==1 & dim(matchresult_2)[1]==0 & dim(truematchresult_1)[1]==1 & dim(truematchresult_2)[1]==0){
    ...
    for (i in 1:(dim(mat_prob_form_0_1)[1]-1)){
     ...
     for (j in (i+1):dim(mat_prob_form_0_1)[1]){
      ...
      colnames(all_true_match_all_branch_sub) = c("gene_ID1","gene_ID2","first_br_indexn","second_br_indexn")
      all_true_match_all_branch_sub=rbind(all_true_match_all_branch_sub, cbind(gene_result,truematchresult))
  ...
  all_pair_result[indexn,]=c(dim(unique_all_match_all_branch_sub)[1],dim(unique_all_true_match_all_branch_sub)[1],numSites,alpha,missingindex,Genotype="Ternary",lost_rate=0,lost_br=0,Method="MO")
...
all_pair_result_form_0_1_sub = sprintf('./FiguresS4_S5_Scenarios5_6/Scenario6_result_rmd/MO_Ternary/summary/ternary_all_alpha_0%s_beta_0%s_%s.csv',alpha_str,alpha_str,numSites)
