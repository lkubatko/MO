...
binary_folder_form = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Binary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)
ternary_folder_form = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Ternary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)

binary_folder_form_10 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_10/Binary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)
ternary_folder_form_10 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_10/Ternary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)

binary_folder_form_20 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_20/Binary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)
ternary_folder_form_20 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_20/Ternary_alpha0%s_beta0%s_sites%s',alpha_str, beta_str,numSites)
...
for (indexn in 1:100){
  ...
  obs_form_0_1 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Obs_0_1_alpha0%s_beta0%s/obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
  obs_form_0_2 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Obs_0_2_alpha0%s_beta0%s/obs_0_2_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
  obs_form_0_1_2 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Obs_0_1_2_alpha0%s_beta0%s/obs_0_1_2_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
  ...
  all_prob_mute_mat_total=c()
 
  #create all possible result
  for (i in 1:length(sampletr$edge.length)){
    
    print(c(indexn,i))
    
    
    possible_mat_obs_form_0_1 = t(as.matrix(mat_obs_form_0_1[i,]))
    
    possible_mat_obs_form_0_2 = t(as.matrix(mat_obs_form_0_2[i,]))
    
    possible_mat_obs_form_0_1_2 = mat_obs_form_0_1_2[[i]]
    
    possible_mat_obs_form = as.data.frame(rbind(possible_mat_obs_form_0_1,possible_mat_obs_form_0_2,possible_mat_obs_form_0_1_2))
    
    prob_mutate = find_mutation_prob_Bj(sampletr$edge.length, unit_theta,unit_gamma,unit_mu,i)
    prob_mute_mat=rbind(prob_mutate$prob_0_1,prob_mutate$prob_0_2,prob_mutate$prob_0_1_2)
    prob_mute_mat_total=sum(as.data.frame(prob_mute_mat)$Mutation_Prob)
    all_prob_mute_mat_total[i]= as.data.frame(prob_mute_mat)$Mutation_Prob[1]
    }
  
  #sample each possible genetype
  selected_br = sample(1:number_br,numSites,prob=all_prob_mute_mat_total/sum(all_prob_mute_mat_total),replace=TRUE)
  
  transformed_obs_dat_Ternary=data.frame(matrix(, nrow=0, ncol=0))
  transformed_obs_dat_Ternary_20=data.frame(matrix(, nrow=0, ncol=0))
  transformed_obs_dat_Ternary_10=data.frame(matrix(, nrow=0, ncol=0))
  
  transformed_obs_dat_Binary=data.frame(matrix(, nrow=0, ncol=0))
  transformed_obs_dat_Binary_20=data.frame(matrix(, nrow=0, ncol=0))
  transformed_obs_dat_Binary_10=data.frame(matrix(, nrow=0, ncol=0))
  
  for (j in 1:length(selected_br)){
    selected_br_num = selected_br[j]
    print(c(indexn,j))
    
    missing_index_20 = sample(1:number_cell, number_cell*0.2, replace=FALSE)
    missing_index_10 = sample(missing_index_20, number_cell*0.1, replace=FALSE)
    
    possible_mat_obs_form_0_1 = t(as.matrix(mat_obs_form_0_1[selected_br_num,]))
    
    possible_mat_obs_form_0_2 = t(as.matrix(mat_obs_form_0_2[selected_br_num,]))
    
    possible_mat_obs_form_0_1_2 = mat_obs_form_0_1_2[[selected_br_num]]
    
    possible_mat_obs_form = as.data.frame(rbind(possible_mat_obs_form_0_1,possible_mat_obs_form_0_2,possible_mat_obs_form_0_1_2))
    
    prob_mutate = find_mutation_prob_Bj(sampletr$edge.length, unit_theta,unit_gamma,unit_mu,selected_br_num)
    prob_mute_mat=rbind(prob_mutate$prob_0_1,prob_mutate$prob_0_2,prob_mutate$prob_0_1_2)
    
    sampleDist = function(n) { sample(x = prob_mute_mat[,2], n, replace = T, prob = prob_mute_mat[,3]/(sum(prob_mute_mat[,3])))}
    
    transformed_obs_Ternary = possible_mat_obs_form[ which(possible_mat_obs_form$Second_branch==sampleDist(1)), ]
    
    transformed_obs_Ternary_10 = transformed_obs_Ternary
    transformed_obs_Ternary_20 = transformed_obs_Ternary
    
    transformed_obs_Ternary_10[1,missing_index_10+2]=c("3")
    transformed_obs_Ternary_20[1,missing_index_20+2]=c("3")
    #transformed_obs_10[1,missing_index[1]+2] = c("-")
    transformed_obs_dat_Ternary=rbind(transformed_obs_dat_Ternary,transformed_obs_Ternary)
    transformed_obs_dat_Ternary_10=rbind(transformed_obs_dat_Ternary_10,transformed_obs_Ternary_10)
    transformed_obs_dat_Ternary_20=rbind(transformed_obs_dat_Ternary_20,transformed_obs_Ternary_20)
    #binary
    transformed_obs_Binary = transformed_obs_Ternary
    transformed_obs_Binary_10 = transformed_obs_Ternary_10
    transformed_obs_Binary_20 = transformed_obs_Ternary_20
    for (binRow in 1:dim(transformed_obs_Binary)[1]){
      for(binCol in 3:dim(transformed_obs_Binary)[2]){
        
        if(transformed_obs_Ternary[binRow,binCol] == 2){transformed_obs_Binary[binRow,binCol]=1}
        if(transformed_obs_Ternary_10[binRow,binCol] == 2){transformed_obs_Binary_10[binRow,binCol]=1}
        if(transformed_obs_Ternary_20[binRow,binCol] == 2){transformed_obs_Binary_20[binRow,binCol]=1}
      }
    }
    #transformed_obs_20[1,missing_index+2] = c("-")
    transformed_obs_dat_Binary=rbind(transformed_obs_dat_Binary,transformed_obs_Binary)
    transformed_obs_dat_Binary_10=rbind(transformed_obs_dat_Binary_10,transformed_obs_Binary_10)
    transformed_obs_dat_Binary_20=rbind(transformed_obs_dat_Binary_20,transformed_obs_Binary_20)
  }
  ...
  #complete
  binary_transformed_obs_dat_out = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Binary_alpha0%s_beta0%s_sites%s/binary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Binary,file = binary_transformed_obs_dat_out)
  ternary_transformed_obs_dat_out = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData/Ternary_alpha0%s_beta0%s_sites%s/ternary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Ternary,file = ternary_transformed_obs_dat_out)
  
  #10missing
  binary_transformed_obs_dat_out_10 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_10/Binary_alpha0%s_beta0%s_sites%s/binary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Binary_10,file = binary_transformed_obs_dat_out_10)
  ternary_transformed_obs_dat_out_10 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_10/Ternary_alpha0%s_beta0%s_sites%s/ternary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Ternary_10,file = ternary_transformed_obs_dat_out_10)
  
  #20 missing
  binary_transformed_obs_dat_out_20 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_20/Binary_alpha0%s_beta0%s_sites%s/binary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Binary_20,file = binary_transformed_obs_dat_out_20)
  ternary_transformed_obs_dat_out_20 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_20/Ternary_alpha0%s_beta0%s_sites%s/ternary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  write.csv(transformed_obs_dat_Ternary_20,file = ternary_transformed_obs_dat_out_20)
