source("/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Ternary/MO_ternary_function_Missing.R")
...
ternary_folder_form_result = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Ternary/Ternary_alpha0%s_beta0%s_%s_result',alpha_str, beta_str,numSites)
...
for (indexn in 1:100){
  ...
  obs_ternary_folder_form_result_br = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/SimulateData_20/Ternary_alpha0%s_beta0%s_sites%s/ternary_obs_0_1_tip_alpha_0%s_beta_0%s_matrix%s.csv', alpha_str, beta_str, numSites,alpha_str, beta_str,indexn)
  ...
  if (dim(mat_obs_form_0_1_2)[1]>0){
    ...
    for (i in 1:dim(initial_obs_0_1)[1]){
      ...
      #rd_unit_mu <- rbeta(10, 100*unit_mu, 100*(1-unit_mu))
        ...
        generate_prob_br <- generate_prob_Missing(sequencing_error_model,unit_theta,unit_gamma,unit_mu,number_br,number_cell,
                                          normal_genotype_0_1_2[i],mutation_genotype_0_1_2[i],initial_obs_0_1[i,],sampletr)
        generate_prob_br_0_1_single <- c(generate_prob_br[,1])
        generate_prob_br_0_2_single <- c(generate_prob_br[,2])
        generate_prob_br_0_1_2_single <- c(generate_prob_br[,3])
        generate_prob_br_all_single <- c(generate_prob_br[,4])
  ...
  ternary_prob_matrix_all_0_1_2_out = sprintf("/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Ternary/Ternary_alpha0%s_beta0%s_%s_result/all_ternary_prob_matrix_all_0_1_2_out_matrix%s_20.csv",alpha_str,alpha_str,numSites,indexn)
...
write.csv(All_loc_dat,file="/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Ternary/All_MO_ternary_location_accuracy_20_sitres80.csv")
