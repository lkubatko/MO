...
source("/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Binary/MO_binary_function_Missing.R")
  ...
  for (indexn in 1:100){
    ...
    for ( nr in 1:dim(initial_obs_0_1)[1]){
      for ( nc in 1:dim(initial_obs_0_1)[2]){
        ...
        if (initial_obs_0_1[nr,nc] == "3"){initial_obs_0_1_recode[nr,nc] ="3"}
      }
    }
    ...
    for (i in 1:dim(initial_obs_0_1_recode)[1]){
      ...
      generate_prob_br <- generate_prob_binary_Missing(alpha,beta,unit_theta+unit_gamma,number_br,number_cell,
    ...
    binary_prob_matrix_all_0_1_out = sprintf("/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Binary/Binary_alpha0%s_beta0%s_%s_result/all_binary_prob_matrix_all_0_1_out_matrix%s_20.csv",alpha_str,alpha_str,numSites,indexn)
...
write.csv(All_loc_dat,file="/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting/MO_Binary/All_MO_binary_location_accuracy_20.csv")
