...
simulate_sample <- function(sequencing_error_model,unit_theta,unit_gamma,unit_mu,number_br,
                            number_cell,ts){
  ...
  #true_0_1_2_tip_matrix = matrix(rep(0,length(ts$tip.label)*length(ts$edge.length)),nrow=length(ts$edge.length),ncol = length(ts$tip.label))
  ...
  for (i in 1:length(ts$edge.length)){
    ...
    trud_possible_tip_all=find_all_possible_mutation_matrix(ts,i)
    true_0_1_tip_matrix[i,]=(trud_possible_tip_all[[1]])
    true_0_2_tip_matrix[i,]=(trud_possible_tip_all[[2]])
    
    true_0_1_2_tip_matrix[[i]]=(trud_possible_tip_all[[3]])
    
    obs_0_1_2_tip_matrix[[i]]= (trud_possible_tip_all[[3]])
  }
  ...
  for (i in 1:length(ts$edge.length)){
    for (j in 1:length(ts$tip.label))
      ...
      if(true_0_1_tip_matrix[i,j+2]==1){obs_0_1_tip_matrix[i,j+2]=sampleDist_1(1)}
    if(true_0_1_tip_matrix[i,j+2]==0){obs_0_1_tip_matrix[i,j+2]=sampleDist_0(1)}
    else{obs_0_1_tip_matrix[i,j+2]=true_0_1_tip_matrix[i,j+2]}
  }
  ...
  for (i in 1:length(ts$edge.length)){
    for (j in 1:length(ts$tip.label))
      ...
      if(true_0_2_tip_matrix[i,j+2]==2){ obs_0_2_tip_matrix[i,j+2]=sampleDist_2(1)}
    if(true_0_2_tip_matrix[i,j+2]==0){obs_0_2_tip_matrix[i,j+2]=sampleDist_0(1)}
    else{obs_0_2_tip_matrix[i,j+2]=true_0_2_tip_matrix[i,j+2]}
  }
  ...
  for (k in 1:length(true_0_1_2_tip_matrix)){
    ...
    {for (i in 1:dim(true_0_1_2_tip_matrix[[k]])[1]){
      ...
      for (j in 1:length(ts$tip.label)){
        ...
        if(true_0_1_2_tip_matrix[[k]][i,j+2]==2){ obs_0_1_2_tip_matrix[[k]][i,j+2]=sampleDist_2(1)}
        
        if(true_0_1_2_tip_matrix[[k]][i,j+2]==1){obs_0_1_2_tip_matrix[[k]][i,j+2]=sampleDist_1(1)}
        
        if(true_0_1_2_tip_matrix[[k]][i,j+2]==0){obs_0_1_2_tip_matrix[[k]][i,j+2]=sampleDist_0(1)}
        
        else{obs_0_1_2_tip_matrix[[k]][i,j+2]=true_0_1_2_tip_matrix[[k]][i,j+2]}
        
      }
    }
    }
    
    else{obs_0_1_2_tip_matrix[[k]]=true_0_1_2_tip_matrix[[k]]}
  }
...
for(i in 2:dim(parameter_setting)[1]){
  
  alpha =   parameter_setting[i,1]
  beta = parameter_setting[i,2]
  ...
  setwd('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting_LargeTree/SimulateData_EXP10')
  ...
  for (indexn in 1:100){
    ...
    form_0_1 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting_LargeTree/SimulateData_EXP10/Obs_0_1_alpha0%s_beta0%s/true_0_1_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
    form_0_2 = sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting_LargeTree/SimulateData_EXP10/Obs_0_2_alpha0%s_beta0%s/true_0_2_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
    form_0_1_2= sprintf('/fs/project/kubatko.2-temp/gao.957/DNA_alignment/bowtie2-2.3.4.2-linux-x86_64/reference/Simulation_Setting_LargeTree/SimulateData_EXP10/Obs_0_1_2_alpha0%s_beta0%s/true_0_1_2_tip_alpha_0%s_beta_0%s_matrix%s.Rdata', alpha_str, beta_str, alpha_str, beta_str,indexn)
    ...
    sim_result = simulate_sample(sequencing_error_model,unit_theta,unit_gamma,unit_mu,length(sampletr$edge.length),length(sampletr$tip.label),sampletr)
