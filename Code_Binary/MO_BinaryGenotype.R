...
# normal_genotype is 0 for binary genotype
# mutation_genotype is 1 for binary genotype
...
generate_prob_binary <- function(alpha,beta,unit_lambda,number_br,number_cell,normal_genotype,mutation_genotype,initial_obs,ts){
  ...
  for (j in c(1:(number_cell))){
    ...
    if (initial_obs[j]==c("-")) { single_tip_exclude=colnames(initial_obs)[j] 
...
for (indexn in 1:10){
  ...
  form = sprintf('RandomTree/RandomTree_%s.tre', indexn)

  sampletr=read.tree(form)
  
  
  obs_form_0_1 = sprintf('Binary_alpha001_beta02/binary_obs_0_1_tip_alpha_001_beta_02_matrix%s.csv', indexn)
  ...
  initial_obs_0_1_recode[ initial_obs_0_1_recode >1  ] <- 1
  ...
  for (i in 1:dim(initial_obs_0_1_recode)[1]){
    ...
    for (j in 1:10){
      ...
      generate_prob_br_all_single <- c(generate_prob_br,rep(0,18-length(generate_prob_br)))
  ...
  binary_prob_matrix_all_0_1_out = sprintf('binary_prob_matrix_all_0_1_out_alpha_001_beta_02_matrix%s.csv', indexn)
  
  
  # binary_prob_matrix_all_0_1 is the probability for all mutations on each branch, each row is for one mutation, and each column is for one branch
  binary_prob_matrix_all_0_1_rownames=cbind(mat_obs_form_0_1[,c(2,3)],binary_prob_matrix_all_0_1)
