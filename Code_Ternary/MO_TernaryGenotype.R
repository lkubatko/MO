...
# alpha is fpr
# beta is fnr
# unit_lambda is transition rate from 0-1
# number_br is number of branches in a tree
# normal_genotype is 0 for ternary genotype
# mutation_genotype can be 1 or 2 for ternary genotype
# initial_obs is the data frame including mutations we want to infer
# ts is the tree on which we will infer the mutation order
# for the ternary genotype, we need to compute the probability assuming three conditions: mutation from 0 to 1;mutation from 0 to 2; mutation from 0 to 1, then 1 to 2
# for the three conditions above, the only difference when using the function below is the mutation_genotype
# mutation_genotype is 1 for mutation from 0 to 1
# mutation_genotype is 2 for the other two conditions
...
# example data analysis for mutation from 0 to 1
# setting parameters
alpha = 0.01
beta = 0.2
...
sampletr=read.tree('RandomTree/RandomTree_1.tre')
load('Obs_0_1_alpha001_beta02/obs_0_1_tip_alpha_001_beta_02_matrix1.Rdata')
  
  
normal_genotype_0_1 = rep(0,18)
mutation_genotype_0_1 = rep(1,18)
initial_obs_0_1 = data.frame(mat_obs_form_0_1[,-c(1,2)])
  
  
  
ternary_prob_matrix_all=c()
ternary_prob_matrix_01=c()
ternary_prob_matrix_02=c()
ternary_prob_matrix_012=c()
...
for (i in 1:dim(initial_obs_0_1)[1]){
    ...
    generate_prob_br_0_1_dat=data.frame(matrix(NA, nrow = 18, ncol = 10))
    generate_prob_br_0_2_dat=data.frame(matrix(NA, nrow = 18, ncol = 10))
    generate_prob_br_0_1_2_dat=data.frame(matrix(NA, nrow = 18, ncol = 10))
    generate_prob_br_all_dat=data.frame(matrix(NA, nrow = 18, ncol = 10))
    
    
    for (j in 1:10){
      
      generate_prob_br <- generate_prob(sequencing_error_model,rd_unit_theta[j],rd_unit_gamma[j],rd_unit_mu[j],number_br,number_cell,
                                        normal_genotype_0_1[i],mutation_genotype_0_1[i],initial_obs_0_1[i,],sampletr)
      generate_prob_br_0_1_single <- c(generate_prob_br[,1],rep(0,18-dim(generate_prob_br)[1]))
      generate_prob_br_0_2_single <- c(generate_prob_br[,2],rep(0,18-dim(generate_prob_br)[1]))
      generate_prob_br_0_1_2_single <- c(generate_prob_br[,3],rep(0,18-dim(generate_prob_br)[1]))
      generate_prob_br_all_single <- c(generate_prob_br[,4],rep(0,18-dim(generate_prob_br)[1]))
    ...
    ternary_prob_matrix_all = rbind(ternary_prob_matrix_all,generate_prob_br_all)
    ternary_prob_matrix_01 = rbind(ternary_prob_matrix_01,generate_prob_br_0_1)
    ternary_prob_matrix_02 = rbind(ternary_prob_matrix_02,generate_prob_br_0_2)
    ternary_prob_matrix_012 = rbind(ternary_prob_matrix_012,generate_prob_br_0_1_2)
  }
 
#ternary_prob_matrix_all is the probability matrix, each row is for one mutation and each column is for one branch
ternary_prob_matrix_all_0_1_rownames=cbind(mat_obs_form_0_1[,c(1)],ternary_prob_matrix_all)
