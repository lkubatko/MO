...
# t is the tree
MO_binary_multinomial<- function(alpha,beta,initial_obs,t){
  ...
  mutate_index = which(obs_genotype=="1"|obs_genotype=="2")
  ...
  #for this locus or location, find the probability of mutation on each branch by the poisson process
    ...
    prob_Bj_final = branch_time_list/(sum(branch_time_list))
